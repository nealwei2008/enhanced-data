...
public abstract class CacheTest {
    ...
    public void cacheTest() {
        ...
        for (int i = 0; i < 10; i++) {
            Assertions.assertNull(demoService.getValue());
        }
        logger.info("同一参数 + 变参数的缓存测试，应仅最多实际调用一次(变参数同一参数一次)");
        for (int i = 0; i < 10; i++) {
            Assertions.assertEquals(2, demoService.getNewValue());
            for (int j = 0; j < 10; j++) {
                Assertions.assertEquals(2 * i, demoService.getNewValue(i));
            }
        }
        logger.info("嵌套缓存测试，应仅最多实际调用一次");
        for (int i = 0; i < 10; i++) {
            Assertions.assertEquals(2, demoService.compose());
        }
